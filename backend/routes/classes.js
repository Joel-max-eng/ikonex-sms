...
router.post('/', async (req, res) => {
  ...
  if (!name || !year_group || !section) return res.status(400).json({ error: 'name, year_group and section are required' });
  try {
    const [result] = await db.query(
      'INSERT INTO class_streams (name, year_group, section, capacity) VALUES (?,?,?,?)',
      [name, year_group, section, capacity || 40]
    );
    res.status(201).json({ id: result.insertId, name, year_group, section, capacity });
  } catch (err) {
    if (err.code === 'ER_DUP_ENTRY') return res.status(409).json({ error: 'Class name already exists' });
...
router.put('/:id', async (req, res) => {
  ...
  try {
    await db.query(
      ...
      [name, year_group, section, capacity, req.params.id]
    );
...
router.delete('/:id', async (req, res) => {
  try {
    const [students] = await db.query('SELECT id FROM students WHERE class_id = ?', [req.params.id]);
    if (students.length) return res.status(409).json({ error: `Cannot delete — ${students.length} student(s) still assigned to this class` });
